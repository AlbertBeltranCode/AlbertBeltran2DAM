...
def generar_ips_ficticias():
    ips_ficticias = []
    ips_bloqueadas = cargar_ips_bloqueadas()  # Cargar las IPs bloqueadas existentes
    ...
    for _ in range(num_ips):
        ...
        if ip in ips_bloqueadas:
            ips_bloqueadas[ip]['count'] += 1  # Incrementar el contador de visitas
        else:
            ips_bloqueadas[ip] = {'count': 1, 'pais': pais}  # Añadir IP nueva con su país y cuenta

    # Abrir el archivo de log y añadir las IPs ficticias sin sobrescribir el archivo
    with open(LOG_LOCAL, 'a') as f:  # Usamos 'a' para añadir al final del archivo
        f.writelines(ips_ficticias)

    # Guardar las IPs bloqueadas con la información de país
    guardar_ips_bloqueadas(ips_bloqueadas)
    print("IPs ficticias añadidas al log y bloqueadas actualizadas.")

# Función para analizar el log y bloquear IPs
def analizar_log():
    generar_ips_ficticias()  # Generar nuevas IPs ficticias
    print("Log analizado y nuevas IPs ficticias generadas.")

# Función para borrar el archivo de log y el archivo de IPs bloqueadas
def borrar_log():
    ...
    if os.path.exists(BLOQUEO_IPS_JSON):
        os.remove(BLOQUEO_IPS_JSON)
        print(f"El archivo {BLOQUEO_IPS_JSON} ha sido borrado.")
    else:
        print(f"El archivo {BLOQUEO_IPS_JSON} no existe.")

# Menú principal
def main():
    while True:
        print("Programa creado por Albert Beltran")
        print("Bienvenido a Ivory, elige una funcion:")
