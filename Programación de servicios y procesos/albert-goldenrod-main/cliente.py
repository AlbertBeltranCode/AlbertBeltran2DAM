...
def start_client():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as client:
        try:
            ...
            print(f"[CONECTADO] Conectado al servidor en {SERVER_HOST}:{SERVER_PORT}")

            while True:
                # Get user input
                message = input("Introduce tu mensaje (escribe 'salir' para salir): ")
                ...
                if not response:
                    print("[DESCONECTADO] Conexion terminada con el servidor.")
                    break
