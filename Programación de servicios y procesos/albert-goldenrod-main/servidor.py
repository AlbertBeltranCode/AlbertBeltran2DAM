...
def handle_client(conn, addr, message_file):
    ...
    with conn:
        try:
            ...
            print(f"[DESCONECTANDO] {addr} conexion reiniciada.")
...
def start_server():
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as server:
        ...
        try:
            ...
            sys.exit(1)
        server.listen()
        print(f"[ESCUCHANDO] El servidor esta escuchando en {HOST}:{PORT}")
        ...
        while True:
            ...
            client_thread.start()
            print(f"[CONEXION ACTIVA] {threading.active_count() - 1}")

if __name__ == "__main__":
    print("[EMPEZANDO] El servidor esta iniciandose...")
    start_server()
