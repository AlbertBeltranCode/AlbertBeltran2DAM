...
const jocarsaMistyrose = (function () {
    ...
    class Chart {
        ...
        createDonutChart(container) {
            ...
            const height = 300;
            const radius = Math.min(width, height) / 2 - 20; // Add padding to prevent cropping
            const innerRadius = radius * 0.6; // Radius for the inner hole
            ...
            dataWithPercentages.forEach((item, index) => {
                ...
                const pathData = `
                    M ${x1} ${y1}
                    A ${radius} ${radius} 0 ${largeArcFlag} 1 ${x2} ${y2}
                    L ${innerRadius * x2 / radius} ${innerRadius * y2 / radius}
                    A ${innerRadius} ${innerRadius} 0 ${largeArcFlag} 0 ${innerRadius * x1 / radius} ${innerRadius * y1 / radius}
                    Z
                `;
                ...
                // Add label to the slice
                const midAngle = (startAngle + endAngle) / 2;
                const labelRadius = (radius + innerRadius) / 2; // Position labels in the middle of the slice
                const labelX = labelRadius * Math.cos((Math.PI * midAngle) / 180);
                const labelY = labelRadius * Math.sin((Math.PI * midAngle) / 180);
        ...
        createLineChart(container) {
            ...
            const yScale = (height - 50) / maxValue;

            let pathData = `M50 ${height - 50}`;

            data.forEach((item, index) => {
                const x = 50 + index * xScale;
                const y = height - 50 - item.value * yScale;
                pathData += ` L${x} ${y}`;
                ...
                point.setAttribute("fill", "hsl(200, 60%, 50%)");
                point.setAttribute("stroke", "#fff");
                point.setAttribute("stroke-width", "2");
                point.style.transition = "fill 0.2s ease";

                // Add hover effect
                point.addEventListener("mouseenter", () => {
                    point.setAttribute("fill", "hsl(200, 60%, 60%)"); // Highlight on hover
                });
                point.addEventListener("mouseleave", () => {
                    point.setAttribute("fill", "hsl(200, 60%, 50%)"); // Restore color
                });
            ...
            line.setAttribute("stroke", "hsl(200, 60%, 50%)");
