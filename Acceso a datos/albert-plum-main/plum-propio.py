...
def export_to_csv(file_structure, csv_file):
    """
    Export the file structure to a CSV file with columns: name, path, type, size.
    """
    with open(csv_file, mode='w', newline='', encoding='utf-8') as file:
        ...
        def write_entries(entries):
            for entry in entries:
                writer.writerow([entry['name'], entry['path'], entry['type'], entry.get('size', 0)])
                if entry['type'] == 'directory' and 'contents' in entry:
                    write_entries(entry['contents'])

        write_entries(file_structure)
...
def main():
    ...
    print(f"Exporting file structure to CSV. Saving to {csv_file}...")
