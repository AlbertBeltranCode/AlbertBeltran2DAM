...
class conexionDB // Defino una nueva clase
{
	...
	public function seleccionaTabla($tabla)
	{
		...
		while ($row = mysqli_fetch_assoc($result)) {
			...
			$fila = []; // Inicializo un array para la fila codificada
			foreach ($row as $clave => $valor) {
				// Itero sobre cada clave y valor de la fila
				$fila[$clave] = $this->codifica($valor); // Codifico el valor y lo añado al array de fila
			}
			$resultado[] = $fila; // Agrego la fila codificada al array de resultados
		}
