...
class conexionDB // Defino una nueva clase
{
	...
	public function seleccionaTabla($tabla) // Método para seleccionar datos de una tabla
	{
		...
		while ($row = mysqli_fetch_assoc($result)) { // Itero sobre cada fila del resultado
			//$resultado[] = $row; // Los añado al array
			$fila = []; // Inicializo un array para la fila
			foreach ($row as $clave => $valor) { // Itero sobre cada clave y valor de la fila
	...
	public function insertaTabla($tabla, $valores) // Método para insertar datos en una tabla
	{
		// Implementar lógica de inserción aquí
	}

	public function actualizaTabla($tabla, $valores) // Método para actualizar datos en una tabla
	{
		// Implementar lógica de actualización aquí
	}
	public function eliminaTabla($tabla, $id) // Método para eliminar datos de una tabla
	{
		// Implementar lógica de eliminación aquí
	}

	private function codifica($entrada) // Método privado para codificar
	{
		return base64_encode($entrada); // Retorno la entrada codificada en base64
	}

	private function decodifica($entrada) // Método privado para decodificar
	{
