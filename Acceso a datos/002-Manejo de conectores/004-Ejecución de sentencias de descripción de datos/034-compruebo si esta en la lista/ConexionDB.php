...
class conexionDB // Defino una nueva clase para manejar la conexión a la base de datos
{
	...
	public function seleccionaTabla($tabla)
	{
			...
			foreach ($row as $clave => $valor) {
				...
				foreach ($restricciones as $restriccion) {
					if ($clave == $restriccion["COLUMN_NAME"]) {
						// Si la columna tiene una restricción, reemplazo el valor con un string indicando que hay datos externos
						$fila[$clave] = "datos externos";
						$pasas = false;
					}
				}
				if ($pasas == true) {
					// Si no hay restricción, guardo el valor original
					$fila[$clave] = $valor;
				}
			}
			$resultado[] = $fila;
		...
		// Codifico el resultado como JSON y lo devuelvo
